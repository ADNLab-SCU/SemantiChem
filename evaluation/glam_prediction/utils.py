...
def random_scaffold_split(dataset, smiles_list, task_idx=None, null_value=0,
    ...
    df1 = pd.DataFrame({'train_idx': train_idx})
    df1.to_csv('random_train_index.csv', index=False)
    df2 = pd.DataFrame({'valid_idx': valid_idx})
    df2.to_csv('random_valid_index.csv', index=False)
    df3 = pd.DataFrame({'test_idx': test_idx})
    df3.to_csv('random_test_index.csv', index=False)
